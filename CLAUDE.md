...
**Prod ports.** Production binding is driven by what's in `system.db`. With ACME configured (`Acme:Domains`/`Acme:Email`/`Acme:AcceptTos` all set), `Program.cs` binds `0.0.0.0:80` + `0.0.0.0:443` and `UseHttpsRedirection` is active. Without ACME — fresh install or LAN-only deployment — Program.cs binds **`0.0.0.0:80` HTTP-only** so the operator can reach `/setup` on first boot, and `UseHttpsRedirection` is skipped (nothing's listening on 443 to redirect to). If `ASPNETCORE_URLS` is set, it wins outright — operator escape hatch for picking a non-80 port. The "post-ACME" → "post-restart-binds-443" transition is why /api/setup returns `restartRequired: true` when ACME fields change.
