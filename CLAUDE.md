...
- `CreateContextConnection(ContextRef.Team(slug))` → `fishbowl-data/teams/{teamId}/team.db` — identical schema to user DBs. Folder is keyed by the team's ULID, not its slug — slug can change (rename) while the folder stays put. `ResolveTeamAsync` translates slug→id before opening.
- Legacy flat layout (`users/{id}.db`, `teams/{id}.db`) auto-migrates on first open — `MigrateLegacyLayoutIfPresent` moves the file into its new folder. Idempotent; never overwrites a pre-existing new-path file.
...
**Dev tools under `tools/`.** `mint-dev-key` issues an API key directly against `ApiKeyRepository` without a browser session — supports `--scopes`, `--context user|team`, `--context-id <slug>`; token on stdout, metadata on stderr. `reindex-dev` calls `NoteRepository.ReEmbedAllAsync` against the disk (bypasses the cookie-only HTTP endpoint). `seed-dev-data` populates notes/todos/contacts in a context for quick UI + MCP iteration; sentinels (`seed:dev` tag on notes, `[seed-dev] ` prefix on contacts/todos) make it idempotent, `--force` overrides. `init-project <slug>` is the [Firepit](https://github.com/chloe-dream/firepit-ai)-integration shortcut: creates (or reuses) a team with the given slug, mints a team-scoped API key with all read/write scopes, and prints both a `cmdkey /generic:firepit/fishbowl-<slug>` line and a Firepit `mcpOverrides` snippet. `snapshot-data` hot-backs-up the whole data dir via SQLite's online backup API to `fishbowl-backups/snapshot-<ts>/` — safe under live writes, intended for cron'd operator runs (see tools/snapshot-data/README.md). All read `system.db` via the real `DatabaseFactory`, share exit codes, and `tools/` is outside the product solution. `.mcp.json` at repo root (gitignored) holds Claude Code's local MCP config pointing at `http://localhost:7181/mcp`; regenerate the token with `mint-dev-key`.

**Firepit / per-project integration.** Each Firepit project that uses Fishbowl as its memory backend maps 1:1 to a Fishbowl **team** (the file-boundary isolation, role-based membership, and team-scoped MCP routes were already in place — "team" is the internal name; "project" is what the user sees on the integration edges). `tools/init-project <slug>` is the bootstrap; the host exposes `/p/<slug>` as a 302 to `/#/team/<slug>/notes` so Firepit's quicklink template (`https://localhost:7180/p/{projectName}` per firepit-ai SPEC §"Quick-Links per Project") doesn't have to know the team-as-project naming choice. There is no `project_id` MCP tool argument and no `X-Project` header — the bearer token's `fishbowl_context_type=team` claim carries the scope. Issue #1 is resolved by this convention.
