...
**Dev tools under `tools/`.** `mint-dev-key` issues an API key directly against `ApiKeyRepository` without a browser session — supports `--scopes`, `--context user|team`, `--context-id <slug>`; token on stdout, metadata on stderr. `reindex-dev` calls `NoteRepository.ReEmbedAllAsync` against the disk (bypasses the cookie-only HTTP endpoint). `seed-dev-data` populates notes/todos/contacts in a context for quick UI + MCP iteration; sentinels (`seed:dev` tag on notes, `[seed-dev] ` prefix on contacts/todos) make it idempotent, `--force` overrides. `init-team <slug>` is the per-team bootstrap: creates (or reuses) a team with the given slug, mints a team-scoped API key with all read/write scopes by default, prints the bearer on stdout + a status banner on stderr. Stays agnostic about who consumes the bearer — downstream agent-CLI wiring (credential storage, MCP config) lives in the consumer's docs, not Fishbowl's. `snapshot-data` hot-backs-up the whole data dir via SQLite's online backup API to `fishbowl-backups/snapshot-<ts>/` — safe under live writes, intended for cron'd operator runs (see tools/snapshot-data/README.md). All read `system.db` via the real `DatabaseFactory`, share exit codes, and `tools/` is outside the product solution. `.mcp.json` at repo root (gitignored) holds Claude Code's local MCP config pointing at `http://localhost:7181/mcp`; regenerate the token with `mint-dev-key`.

**Per-team agent integration.** Any agent CLI that wants its own Fishbowl memory namespace gets its own **team** — file-boundary isolation, role-based membership, team-scoped MCP routes. `tools/init-team <slug>` bootstraps the team plus a team-scoped key. The bearer token's `fishbowl_context_type=team` claim carries the scope; there is no `project_id` MCP tool argument and no `X-Project` header — the token *is* the scope. The host also serves `/t/<slug>` as a 302 to `/#/team/<slug>/notes`, a thin URL-alias convenience for downstream link templates that prefer not to construct the SPA hash route directly.
